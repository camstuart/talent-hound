...
| 5c | An Exa search uses the stored credential | **FAIL until now.** The search client was built once at start-up with an empty key and never touched again, and Search refuses a blank key before making any request — so every search this build could make was refused, whatever the recruiter had stored. The flagship acceptance depends on it: step three previews and sends a query, step four expects ten Active roles. The credential is now read when the search is made. Reaching the provider is still a laptop gate. |
| 5b | The cloud endpoint is contacted at all | **PASS off-laptop** — an OpenAI-compatible transport now sends to the approved endpoint with the credential read at the moment of the request, exercised against a stand-in provider. Previously not wired at all: One client is built, pointed at the local runtime, and handed to the cloud service, so a payload previewed for an endpoint and approved for it was answered on this machine while the record said otherwise. a payload previewed for an endpoint and approved for it was answered on this machine while the record said otherwise. A transport that reports a different endpoint than the approved one is still refused before anything is recorded. Sending to a real provider remains a laptop gate: nothing here has contacted one. |
...
| 8c | Personal data is refused wherever it is entered | **FAIL until now.** The guard was on creating a candidate and on artifacts. Editing a candidate was unguarded, and contacts were unguarded entirely — a contact being a full name, an email address and a phone number, the one record type made only of direct identifiers. The gate can also close after records exist, leaving every one of them an open field. Companies stay allowed: an organization is not a person, the same distinction the search and cloud boundaries draw. |
| 8b | The encryption gate judges the folder the data is in | **FAIL until now.** The gate checked the folder the recruiter chose in the wizard; the database opened where it always had, and the choice was recorded and never read. So a recruiter could point the wizard at an encrypted disk, be told real data was allowed, and have every record written to a folder nobody checked. The gate now judges the folder in use, and the chosen one is opened at the next launch instead of being decorative. |
